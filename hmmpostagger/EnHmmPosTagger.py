...
class EnHmmPosTagger(object):
    ...
    def __init__(self, dic_file: str, file_type: str = 'utf-8') -> None:
        ...
        with open(dic_file, encoding=file_type) as data:
            for line in data:
                ...
                items = line.split('/')  # 将每行拆分成词语和它的词性标注
                word = items[0]  # 获得单词
                pos = items[1]  # 获得词性标注
                # 将词语和词性分别转移到对应的集合中
    ...
    def train(self, train_file: str, file_type: str = 'utf-8') -> None:
        """
        利用语料库文件训练模型
    ...
    def viterbi_predict(self, input_seq) -> str:
        ...
        token_list = word_tokenize(input_seq)  # 获得分词结果
...
if __name__ == '__main__':
    entagger = EnHmmPosTagger()
    entagger.init_restart(r'data.txt')
    entagger.train(r'data.txt')
    sentence = input()
    print(entagger.viterbi_predict(sentence))
